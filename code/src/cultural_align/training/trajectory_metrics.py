...
def _aggregate_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "va95": float(np.nanmean(va95_pred)),
        "va95_true": float(np.nanmean(va95_true)),
        ...
        "rpa": float(np.nanmean(rpa_pred)),
        "rpa_true": float(np.nanmean(rpa_true)),
    ...
    aggregate.update(
        {
            ...
            "trajectory_metric_policy": {
                ...
                "va95": "mean predicted trajectory-level 95th percentile of |v|*|a|; true mean and diagnostic difference fields are also reported",
                "rpa": "mean predicted trajectory-level relative positive acceleration; true mean and diagnostic difference fields are also reported",
