...
TRACK_COLUMNS = ["frame", "id", "x", "y", "xVelocity", "yVelocity", "xAcceleration", "yAcceleration", *EIGHT_ID_COLUMNS]


def scene_id_for_tracks_path(path: Path) -> str:
    prefix = path.stem.split("_")[0]
    return f"DJI_{path.parent.name}_{prefix}"


def vehicle_ids(meta_path: Path) -> set[int]:
    meta = pd.read_csv(meta_path, usecols=["id", "class"])
    return set(int(v) for v in meta.loc[meta["class"].isin(VEHICLE_CLASSES), "id"].to_numpy())
...
def row_feature(row: Any, frame_lookup: dict[int, dict[int, tuple[float, float]]]) -> np.ndarray:
    return np.asarray([float(row.xVelocity), float(row.xAcceleration), float(row.yVelocity), float(row.yAcceleration), *eight_distances(row, frame_lookup)], dtype=np.float32)


def build(args: argparse.Namespace) -> dict[str, Any]:
    started = time.time()
    writer = DatasetWriter(args.out_dir, DATASET, args.shard_size)
    tracks_files = sorted(Path(args.raw_root).glob("DJI_*/*_tracks.csv"))
    if not tracks_files:
        raise FileNotFoundError(f"no DJI *_tracks.csv under {args.raw_root}")
    ...
    for tracks_path in tracks_files:
        if total_kept >= limit:
            break
        prefix = tracks_path.stem.split("_")[0]
        scene_id = scene_id_for_tracks_path(tracks_path)
        assigned = load_assigned_track_ids(args.assignment_dir, DATASET, scene_id)
        vehicle_set = vehicle_ids(tracks_path.parent / f"{prefix}_tracksMeta.csv")
        df = finite_rows(pd.read_csv(tracks_path, usecols=TRACK_COLUMNS), TRACK_COLUMNS)
        before = len(df)
        df = df[df["id"].isin(vehicle_set)].copy()
    ...
    summary = base_summary(DATASET, writer, started, {
        ...
        "label_source": "DJI CSV xAcceleration/yAcceleration; no resampling and no differencing",
        "assignment_filter": "vehicle class plus status=assigned only",
        "neighbor_policy": "DJI explicit eight-neighbor ID columns; same-frame Euclidean distance; missing=-1",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build DJI TrajVista XY/PsiPhi NPZ shards.")
    parser.add_argument("--raw-root", type=Path, required=True, help="DJI root containing DJI_* recording directories.")
    ...
    parser.add_argument("--abs-ay-limit", type=float, default=8.0)
    parser.add_argument("--delta-ax-limit", type=float, default=6.0)
    parser.add_argument("--delta-ay-limit", type=float, default=6.0)
