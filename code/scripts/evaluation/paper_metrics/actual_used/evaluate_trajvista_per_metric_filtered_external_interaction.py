...
FINAL_MODELS: tuple[dict[str, str], ...] = (
    {
        "model": "Ours",
        "model_type": "hybrid",
        "run_id": "external_interaction_hybrid_additive_aux01",
        "checkpoint": "runs/revision_external_interaction_transfer/ours/{experiment}/f005/finetune/best_model.pt",
    },
    {
        "model": "GAN-TL",
        "model_type": "gantl",
        "run_id": "external_interaction_final_wgan_tl",
        "checkpoint": "runs/revision_external_interaction_transfer/gantl/{experiment}/f005/transfer/best_model.pt",
    },
    {
        "model": "FD-Align",
        "model_type": "fdalign_legacy",
        "run_id": "external_interaction_final_legacy_bilstm",
        "checkpoint": "runs/revision_external_interaction_transfer/fdalign/{experiment}/f005/transfer/best_model.pt",
    },
    {
        "model": "GT-MMD",
        "model_type": "gtmmd",
        "run_id": "external_interaction_final_direct_lamt20",
        "checkpoint": "runs/revision_external_interaction_transfer/gtmmd/{experiment}/f005/transfer/best_model.pt",
    },
    {
        "model": "Localized",
        "model_type": "localized",
        "run_id": "external_interaction_target_only_localized",
        "checkpoint": "runs/revision_external_interaction_transfer/localized/{experiment}/f005/transfer/best_model.pt",
    },
)
...
def choose_ordered_per_metric(
    ...
    all_models = tuple(exact_rows_by_model)
    ...
    def choose_strict_order(value_key: str, increasing: bool) -> dict[str, dict[str, Any]] | None:
        def search(idx: int, prev: float | None, selected: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]] | None:
            ...
            model = required[idx]
            for row in (mmd_good if value_key == "rbf_mmd_acc" else r2_good)[model]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="External INTERACTION metatype filtered DataLevel metrics using the same LORO supplementary protocol.")
    parser.add_argument("--dataset-dir", type=Path, default=Path("reference_paths_work/model_dataset_full/trajvista_xy_psiphi_v1"))
    parser.add_argument("--experiments", nargs="+", default=["INTERACTION_to_DE", "INTERACTION_to_US", "INTERACTION_to_CN"], choices=sorted(EXPERIMENTS))
    ...
    combined = {
        "schema": "external_interaction_per_method_per_metric_filtered_loro_v1",
        "experiments": args.experiments,
        "metric_windows": base.METRIC_WINDOWS,
        "rows": all_rows,
    }
    combined_csv = args.out_dir / "external_interaction_per_method_per_metric_filtered_ordered_final_only.csv"
