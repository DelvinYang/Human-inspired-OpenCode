...
find_project_root <- function(script_dir) {
  ...
  for (path in candidates) {
    if (file.exists(file.path(path, "pyproject.toml")) &&
        file.exists(file.path(path, "README.md"))) {
      return(normalizePath(path))
    }
  }
  stop("Cannot locate repository root containing pyproject.toml and README.md")
}
...
load_longtail_case_main_figure_data <- function() {
  ...
  if (output_dir == "") {
    output_dir <- file.path(project_root, "artifacts", "longtail_case_demo")
  }
